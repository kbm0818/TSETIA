...
dtStatus dtNavMesh::init(const dtNavMeshParams* params)
{
	...
	// Only allow 31 salt bits, since the salt mask is calculated using 32bit unsigned int and it will overflow.
			...
			for (int k = 0; k < cons.size(); ++k)
			{
				...
				if (idx != DT_NULL_LINK)
				{
					...
					if (dir == 0 || dir == 4)
					{
						...
						if (tmin > tmax)
							dtSwap(tmin,tmax);
						link->bmin = (unsigned char)floorf(dtClamp(tmin, 0.0f, 1.0f)*255.0f);
						link->bmax = (unsigned char)ceilf(dtClamp(tmax, 0.0f, 1.0f)*255.0f);
					}
					else if (dir == 2 || dir == 6)
					{
