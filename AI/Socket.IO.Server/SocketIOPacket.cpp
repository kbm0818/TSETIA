...
namespace sio
{
	...
	void accept_float_message(int_message const& msg, Value& val)
	{
		val(msg.get_int());
	}
	...
	void accept_binary_message(binary_message const& msg, Value& val, Document& doc, vector<shared_ptr<const string> >& buffers)
	{
		...
		char frame_char = SocketIOPacket::frame_message;
	...
	SocketIOPacket::SocketIOPacket(string const& nsp, message::ptr const& msg, int pack_id, bool isAck) :
	...
	SocketIOPacket::SocketIOPacket(type type, string const& nsp, message::ptr const& msg) :
		_frame(frame_message),
		_type(type),
		_nsp(nsp),
		_message(msg),
	...
	SocketIOPacket::SocketIOPacket(SocketIOPacket::frame_type frame) :
		_frame(frame),
	...
	bool SocketIOPacket::is_binary_message(string const& payload_ptr)
	{
		return payload_ptr.size()>0 && payload_ptr[0] == frame_message;
	}

	bool SocketIOPacket::is_text_message(string const& payload_ptr)
	{
		return payload_ptr.size()>0 && payload_ptr[0] == (frame_message + '0');
	}

	bool SocketIOPacket::is_message(string const& payload_ptr)
	{
		return is_binary_message(payload_ptr) || is_text_message(payload_ptr);
	}

	bool SocketIOPacket::parse_buffer(const string &buf_payload)
	{
	...
	bool SocketIOPacket::parse(const string& payload_ptr)
	{
		assert(!is_binary_message(payload_ptr)); //this is ensured by outside
		_frame = (SocketIOPacket::frame_type) (payload_ptr[0] - '0');
		_message.reset();
		_pack_id = -1;
		_buffers.clear();
		_pending_buffers = 0;
		...
		if (_frame == frame_message) {
			_type = (SocketIOPacket::type)(payload_ptr[pos] - '0');
	...
	bool SocketIOPacket::accept(string& payload_ptr, vector<shared_ptr<const string> >&buffers)
	{
	...
	SocketIOPacket::frame_type SocketIOPacket::get_frame() const
	{
		return _frame;
	}

	SocketIOPacket::type SocketIOPacket::get_type() const
	{
		assert((_type & type_undetermined) == 0);
		return (type)_type;
	}

	string const& SocketIOPacket::get_nsp() const
	{
		return _nsp;
	}

	message::ptr const& SocketIOPacket::get_message() const
	{
		return _message;
	}

	unsigned SocketIOPacket::get_pack_id() const
	{
		return _pack_id;
	}


	void PacketManager::SetDecodeCallback(function<void(SocketIOPacket const&)> const& decode_callback)
	{
		m_decode_callback = decode_callback;
	}

	void PacketManager::SetEncodeCallback(function<void(bool, shared_ptr<const string> const&)> const& encode_callback)
	{
	...
	void PacketManager::encode(SocketIOPacket& pack, encode_callback_function const& override_encode_callback) const
	{
	...
	void PacketManager::PutPayLoad(string const& payload)
	{
		unique_ptr<SocketIOPacket> p;
		do
		{
			if (SocketIOPacket::is_text_message(payload))
			{
				p.reset(new SocketIOPacket());
				if (p->parse(payload))
				{
			...
			else if (SocketIOPacket::is_binary_message(payload))
			{
				if (m_partial_packet)
				{
