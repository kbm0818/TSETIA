	...
	dtNavMeshQuery* navquery, const dtQueryFilter* filter)
{
	...
	m_npolys = std::min((int)Data.Polys.size(), MAX_LOCAL_POLYS);

	int PolyIdx = 0;
	for (auto It = Data.Polys.begin(); It < Data.Polys.end(); ++It)
	{
		m_polys[PolyIdx] = *It;
	...
	std::vector<dtPolyRef> PathLookup;
	for (int Idx = 0; Idx < npath; Idx++)
	{
		PathLookup.push_back(path[Idx]);
	}

	m_nsegs = 0;
	for (size_t Idx = 0; Idx < Data.Edges.size(); Idx++)
	{
		float tseg = 0.0f;
		...
		bool check = true;
		auto It = std::find(PathLookup.begin(), PathLookup.end(), Data.Edges[Idx].p0);
		if (It == PathLookup.end())
		{
			check &= false;
		}
		It = std::find(PathLookup.begin(), PathLookup.end(), Data.Edges[Idx].p1);
		if (It == PathLookup.end())
		{
			check &= false;
		}
