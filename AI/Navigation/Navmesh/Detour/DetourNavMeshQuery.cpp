...
#include "Navmesh/Detour/DetourAssert.h"
											...
											const float* referencePt, float tolerance) const
{
	...
	*nearestRef = 0;
	...
	int bestPolyInTolerance = -1;
	int bestPolyOutside = -1;
								  ...
								  dtQueryResult& result, float* totalCost) const
{
	...
	if (startRef == endRef)
	{
		result.addItem(startRef, 0.0f, startPos, 0);
		return DT_SUCCESS;
	}
...
//@UE4 END
	...
	do
	{
		result.addItem(node->id, node->cost - prevCost, node->pos, 0);
		prevCost = node->cost;
	...
	float dtNavMeshQuery::CalcSegmentCostOnPoly(uint64_t PolyID, const dtQueryFilter * Filter, const FVector & StartLoc, const FVector & EndLoc) const
	{
		unsigned char AreaID = RECAST_DEFAULT_AREA;
		m_nav->getPolyArea(PolyID, &AreaID);
	...
	float* resultWalls, dtPolyRef* resultRefs, int* resultCount, const int maxResult)
{
	if (*resultCount < maxResult)
	{
		...
		if (distSqr <= radiusSqr)
		{
			const int Wall0Offset = (*resultCount * 6) + 0;
			const int Wall1Offset = (*resultCount * 6) + 3;
