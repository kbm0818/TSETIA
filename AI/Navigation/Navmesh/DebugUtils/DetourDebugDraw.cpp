	...
	if (flags & DU_DRAWNAVMESH_OFFMESHCONS)
	{
		...
		for (int i = 0; i < tile->header->polyCount; ++i)
		{
			...
			if (query && query->isInClosedList(base | (dtPolyRef)i))
				...
				col = duDarkenCol( intToCol(p->getArea(), 220));
		...
		for (int i = 0; i < tile->header->polyCount; ++i)
		{
			...
			if (p->getType() == DT_POLYTYPE_OFFMESH_SEGMENT)
			{
				...
				unsigned int col = duDarkenCol( intToCol(p->getArea(), 220));
...
void duDebugDrawNavMeshClusters(struct duDebugDraw* dd, const dtNavMesh& mesh)
{
	...
	for (int i = 0; i < mesh.getMaxTiles(); ++i)
	{
		...
		{
			duDebugDrawNavMeshPoly(dd, mesh, base | (dtPolyRef)ip,  intToCol(tile->polyClusters[ip],255));
		}
	}
} 
	...
	for (int y = 0; y < h; ++y)
	{
		for (int x = 0; x < w; ++x)
		{
			...
			if (area == 63)
				...
				col = duLerpCol(color,  intToCol(area, 255), 32);
...
void duDebugDrawTileCacheLayerRegions(struct duDebugDraw* dd, const dtTileCacheLayer& layer, const float cs, const float ch)
{
	...
	for (int y = 0; y < h; ++y)
	{
		for (int x = 0; x < w; ++x)
		{
			...
			unsigned int col = duLerpCol(color,  intToCol(reg, 255), 192);
								  ...
								  const float* orig, const float cs, const float ch)
{
	...
	for (int i = 0; i < lcset.nconts; ++i)
	{
		...
		for (int j = 0; j < c.nverts; ++j)
		{
			...
			color = duDarkenCol( intToCol(i, a));
			if (va[3] & 0x80)
			{
				// Border vertex
								  ...
								  const float* orig, const float cs, const float ch)
{
	...
	for (int i = 0; i < lmesh.npolys; ++i)
	{
		...
		if (lmesh.areas[i] == DT_TILECACHE_WALKABLE_AREA)
			...
			color =  intToCol(lmesh.areas[i], 255);
