...
protected:
	friend class Tester;
	friend class Tester_fully;
	friend class DtMeshLoader;
...
struct  dtQueryResult
{
	...
	bool operator ==(const dtQueryResult &R) const {
		if (this->data.size() != R.data.size())
			return false;

		for (int i = 0; i < this->data.size(); i++)
		{
			bool result = true;
			//result &= (data[i].cost == R.data[i].cost);
			//result &= (data[i].flag == R.data[i].flag);
			//result &= (data[i].pos[0] == R.data[i].pos[0]);
			//result &= (data[i].pos[1] == R.data[i].pos[1]);
			//result &= (data[i].pos[2] == R.data[i].pos[2]);
			result &= (data[i].ref == R.data[i].ref);
			if (result == false)
				return false;
		}
...
protected:
	...
	inline int addItem(dtPolyRef ref, float cost, const float* pos, unsigned int flag) { 
		data.push(dtQueryResultPack(ref, cost, pos, flag)); 
		return data.size() - 1; 
	}
...
public:
	...
	float CalcSegmentCostOnPoly(uint64_t PolyID, const dtQueryFilter* Filter, const FVector& StartLoc, const FVector& EndLoc) const;
